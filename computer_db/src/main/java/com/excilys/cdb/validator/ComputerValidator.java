	...
	public static Computer validComputer(Computer computer) throws ValidatorStringException, ValidatorDateException {
		...
		if (computer.getDiscontinued() != null && computer.getIntroduced() == null) {
			throw new ValidatorDateException(
					ExceptionHandler.getMessage(MessageException.COMPUTER_DISCONTINUED_ALONE, null));
		}
		...
		if (computer.getIntroduced() != null && computer.getDiscontinued() != null
			...
			throw new ValidatorDateException(
					ExceptionHandler.getMessage(MessageException.COMPUTER_INTRODUCED_AFTER, null));
		}
		return computer;
	}
	...
	public static String validName(String name) throws ValidatorStringException {
		if (name == null) {
			throw new ValidatorStringException(
					ExceptionHandler.getMessage(MessageException.VALIDATION_NAME_NULL, null));
		}
		if (!SecurityTextValidation.valideString(name)) {
			throw new ValidatorStringException(ExceptionHandler.getMessage(MessageException.SPECIAL_CHARACTERS, null));
		}
		if (name.length() < TAILLE_MIN_NAME) {
			throw new ValidatorStringException(ExceptionHandler.getMessage(MessageException.VALIDATION_NAME_SHORT,
					Arrays.asList(name, TAILLE_MIN_NAME).toArray()));
		}
		if (name.length() > TAILLE_MAX_NAME) {
			throw new ValidatorStringException(ExceptionHandler.getMessage(MessageException.VALIDATION_NAME_LONG,
					Arrays.asList(name, TAILLE_MIN_NAME).toArray()));
		}
		return name;
	}
	...
	public static LocalDate validIntroduced(final LocalDate introduced) throws ValidatorDateException {
		if (introduced != null) {
			if (!DateValidation.validDateInBetween(introduced, BEGIN_DATE_VALID, END_DATE_VALID)) {
				...
				Object[] o = Arrays.asList(BEGIN_DATE_VALID.format(formatter), END_DATE_VALID.format(formatter))
						.toArray();
				throw new ValidatorDateException(
						ExceptionHandler.getMessage(MessageException.VALIDATION_DATE_INTRODUCED, o));
			}
		}
		return introduced;
	}
		...
		if (discontinued != null) {
			if (!DateValidation.validDateInBetween(discontinued, BEGIN_DATE_VALID, END_DATE_VALID)) {
				...
				Object[] o = Arrays.asList(BEGIN_DATE_VALID.format(formatter), END_DATE_VALID.format(formatter))
						.toArray();
				throw new ValidatorDateException(
						ExceptionHandler.getMessage(MessageException.VALIDATION_DATE_DISCONTINUED, o));
			}
		}
		return discontinued;
	}
